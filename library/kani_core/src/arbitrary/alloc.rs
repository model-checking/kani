...
//! This macro generates the `Arbitrary` and `BoundedArbitrary` implementations and the models
//! that need an allocator: `Box`, `Rc`, `Arc`, `Vec`, `String`, `BTreeMap` and `BTreeSet`, and the
//! unbounded slice and `Vec` models. The `kani` library invokes it with `alloc_path` bound to
//! `std`; `kani_lib!(alloc)` invokes it inside `alloc` itself, where these types are local, so
//! that the `verify-std` flow has them too.

#[macro_export]
macro_rules! generate_alloc_arbitrary {
    () => {
        ...
        /// instead.)
        /// These models are *optional*: they require `alloc`, so `core::kani` does not define them,
        ...
        ///
        /// This model is *optional*: it requires `alloc`, so `core::kani` does not define it,
        /// c.f. `KaniModel::is_optional`.
        ...
        /// allocator with exactly the layout `Vec`'s safety contract requires for `capacity`, so `Vec`
        /// frees it correctly on drop.
        ///
