...
//! This module introduces `Arbitrary` implementations for `std`-only types. The trait itself, the
//! `core` implementations and the `alloc` ones (`Vec`, `String`, `Box`, `Rc`, `Arc`) come from
//! `kani_core`, c.f. `kani_core::kani_lib!(kani)`.
