...
impl<T: kani::Arbitrary + Clone + fmt::Debug> Strategy for Just<T> {
    type Tree = Self;
    type Value = T;
...
impl<T: kani::Arbitrary + Clone + fmt::Debug> ValueTree for Just<T> {
    type Value = T;
    noshrink!();
    fn current(&self) -> T {
        self.0.clone()
...
impl<T: kani::Arbitrary + fmt::Debug, F: Clone + Fn() -> T> Strategy for LazyJust<T, F> {
    type Tree = Self;
    type Value = T;
...
impl<T: kani::Arbitrary + fmt::Debug, F: Fn() -> T> ValueTree for LazyJust<T, F> {
    type Value = T;
    noshrink!();
...
impl<T, F: Clone + Fn() -> T> Clone for LazyJust<T, F> {
    fn clone(&self) -> Self {
        Self { _function: self._function.clone() }
    }
}
