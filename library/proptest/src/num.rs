...
pub(crate) fn sample_uniform<X: kani::Arbitrary + Ord>(run: &mut TestRunner, range: Range<X>) -> X {
...
pub(crate) fn sample_uniform_incl<X: kani::Arbitrary + Ord>(run: &mut TestRunner, start: X, end: X) -> X {
    let value: X = kani::any();
...
//                 /// Creates a new binary searcher starting at the given value.
//                 pub fn new(start: $typ) -> Self {
//                     BinarySearch { lo: 0.0, curr: start, hi: start, allowed: FloatTypes::all() }
//                 }

//                 fn new_with_types(start: $typ, allowed: FloatTypes) -> Self {
//                     BinarySearch { lo: 0.0, curr: start, hi: start, allowed }
//                 }
...
//                 fn new_clamped(lo: $typ, start: $typ, hi: $typ) -> Self {
//                     BinarySearch {
//                         lo: if start.is_sign_negative() { hi.min(0.0) } else { lo.max(0.0) },
//                         hi: start,
//                         curr: start,
...
//                     let new_mid =
//                         if new_mid == self.curr || 0.0 == interval { new_mid } else { self.lo };
...
//                     self.lo = if self.curr == self.lo { self.hi } else { self.curr };
...
//             while state.simplify() {
//                 assert!(state.current() < -42, "Violated bounds: {}", state.current());
//             }
...
//         let mut runner = TestRunner::default();
//         let mut value = (::std::f64::MIN_POSITIVE..2.0).new_tree(&mut runner).unwrap();
