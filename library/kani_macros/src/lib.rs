...
///
/// When using `#[derive(Arbitrary)]` on a struct, the `#[invariant(<cond>)]`
...
/// type-safe values, **adding `#[invariant(...)]` to any fields will further
...
/// ```
///
/// Therefore, using the `#[invariant(...)]` attribute can lead to vacuous
...
/// #[derive(kani::Arbitrary)]
/// struct AlwaysPositive {
///     #[invariant(*inner >= 0 && *inner < i32::MIN)]
///     inner: i32,
/// }
///
/// #[kani::proof]
...
///
/// When using `#[derive(Invariant)]` on a struct, the `#[invariant(<cond>)]`
/// attribute can be added to its fields to indicate a type safety invariant
