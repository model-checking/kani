...
use syn::{BinOp, Expr, ExprBinary, Stmt, Block, visit_mut::VisitMut, Ident};
...
impl CallReplacer {
    ...
    fn should_replace(&self, expr_path: &syn::ExprPath) -> bool {
        ...
        let full_path = expr_path.path.segments
            .iter()
...
fn transform_function_calls(expr: Expr, function_name: &str, var_prefix: String) -> TransformationResult {
    ...
    let declarations: Vec<Stmt> = newreplace
        .iter()
        .map(|(call, var)| {
            syn::parse_quote!(let mut #var = #call;)
        })
        .collect();
...
pub fn loop_invariant(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let transform_inv: TransformationResult = transform_function_calls(inv_expr.clone(), "old", var_prefix);
