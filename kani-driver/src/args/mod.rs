...
pub struct VerificationArgs {
    ...
    // Note that this argument lives here because each of its flattened arguments start with `--no`,
    // so when displayed with `--help` they're alphabetical.
    /// Disable some of Kani's default checks.
    #[command(flatten)]
