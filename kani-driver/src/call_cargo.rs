...
impl KaniSession {
    ...
    pub fn cargo_build(&self) -> Result<CargoOutputs> {
        ...
        for package in packages {
            for target in package_targets(&self.args, package) {
                ...
                artifacts.append(&mut self.run_cargo(cmd)?);
                found_target = true;
            }
        }
    ...
    fn run_cargo(&self, cargo_cmd: Command) -> Result<Vec<Artifact>> {
            ...
            for message in Message::parse_stream(reader) {
                ...
                match message {
                    ...
                    Message::CompilerArtifact(rustc_artifact) => {
                        artifacts.extend(map_kani_artifact(rustc_artifact).into_iter());
                    }
...
fn map_kani_artifact(rustc_artifact: cargo_metadata::Artifact) -> Option<Artifact> {
    ...
    let result = rustc_artifact.filenames.iter().find_map(|path| {
        if path.extension() == Some("rmeta") {
            ...
            println!("{} Match: {}", path.extension().unwrap(), meta_path.display());
    ...
    println!("Artifact: {rustc_artifact:?} Result: {result:?}");
    result
}
