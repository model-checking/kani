    ...
    for harness in &harnesses {
        ...
        if result == VerificationStatus::Failure {
            ...
            if !has_run_once && ctx.args.gen_exec_trace {
                ...
                let mut bin_cmd = Command::new(bin_file.clone());
                bin_cmd.env("DET_VALS_FILE", det_vals_file.clone());

                let cmd_str = format!("DET_VALS_FILE={} rust-lldb {}", det_vals_file.to_str().unwrap(), bin_file);
                println!("To debug, run '{}'", cmd_str);

                println!("Running concrete executable...");
