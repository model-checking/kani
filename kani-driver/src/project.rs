...
use anyhow::Result;
use kani_metadata::{
    artifact, artifact::convert_type, ArtifactType, ArtifactType::*, HarnessMetadata, KaniMetadata,
};
use std::fs::File;
...
pub fn cargo_project(session: &KaniSession, keep_going: bool) -> Result<Project> {
    ...
    if session.args.function.is_some() {
        ...
        let metadata = metadata_with_function(session, joined_name, merged_metadata, todo!());
...
impl<'a> StandaloneProjectBuilder<'a> {
    ...
    fn build(self) -> Result<Project> {
        ...
        self.session.record_temporary_files(&[&rlib_path, &self.metadata.path]);
        ...
        let artifacts: Vec<_> = metadata
            .test_harnesses
            .iter()
            .chain(metadata.proof_harnesses.iter())
            .filter_map(|harness_metadata| {
                let symtab_out =
                    Artifact::try_new(harness_metadata.goto_file.as_ref()?, SymTabGoto).ok()?;
                let goto_path = convert_type(&symtab_out.path, symtab_out.typ, Goto);
                self.session.record_temporary_files(&[&symtab_out.path, &goto_path]);

                debug!(?symtab_out, "build link");
                self.session.link_goto_binary(&[symtab_out.to_path_buf()], &goto_path).ok()?;
                let goto = Artifact::try_new(&goto_path, Goto).ok()?;

                /// All other harness artifacts that may have been generated as part of the build.
                Some(
                    [symtab_out.clone(), goto]
                        .into_iter()
                        .chain([TypeMap, VTableRestriction, PrettyNameMap].iter().filter_map(
                            |typ| {
                                let artifact = Artifact::try_from(&symtab_out, *typ).ok()?;
                                self.session.record_temporary_file(&artifact.path);
                                Some(artifact)
                            },
                        ))
                        .collect::<Vec<_>>(),
