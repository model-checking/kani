...
- `--format=[pretty|markdown|json]`: choose the output format. The default is `pretty`, which prints to the terminal. `markdown` and `json` write to Markdown and JSON files instead, respectively.
