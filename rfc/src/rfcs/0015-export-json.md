...
- **A path of `-`** — *not* special-cased to mean stdout; it is treated as a literal filename. Streaming
  ...
  half-specified now.
- **A path that already exists as a directory** — rejected at argument-parse time, like `--sarif`.
...
by case, and adding one is a minor schema change (a new field). `cbmc_args` is the honest catch-all for
...
round-tripped byte-for-byte — sufficient for the comparability signal, but a consumer must not treat the
...
Reading `run_state` is therefore not optional for a sound consumer:
...
Kani already emits SARIF, it is stable, and it is *not* `-Z` gated — so the bar for a second
...
does not model, that is a parser-level concern resolved by adding a variant — a *major* schema change
