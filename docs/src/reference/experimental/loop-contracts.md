...
fn simple_loop_with_loop_invariants() {
    let mut x: u64 = kani::any_where(|i| *i >= 1);
...
Now let's run the proof with loop invariants through kani:
``` bash
kani simple_loop_with_loop_invariants.rs  -Z loop-contracts
```
...
Check 10: simple_loop_with_loop_invariants.loop_invariant_base.1
         - Status: SUCCESS
         - Description: "Check invariant before entry for loop simple_loop_with_loop_invariants.0"
         - Location: simple_while_loop.rs:15:5 in function simple_loop_with_loop_invariants

Check 11: simple_loop_with_loop_invariants.loop_assigns.1
         - Status: SUCCESS
         - Description: "Check assigns clause inclusion for loop simple_loop_with_loop_invariants.0"
         - Location: simple_while_loop.rs:15:5 in function simple_loop_with_loop_invariants

Check 13: simple_loop_with_loop_invariants.assigns.1
         - Status: SUCCESS
         - Description: "Check that x is assignable"
         - Location: simple_while_loop.rs:17:9 in function simple_loop_with_loop_invariants

Check 14: simple_loop_with_loop_invariants.loop_invariant_step.1
         - Status: SUCCESS
         - Description: "Check invariant after step for loop simple_loop_with_loop_invariants.0"
         - Location: simple_while_loop.rs:15:5 in function simple_loop_with_loop_invariants

Check 15: simple_loop_with_loop_invariants.loop_invariant_step.2
         - Status: SUCCESS
         - Description: "Check invariant after step for loop simple_loop_with_loop_invariants.0"
         - Location: simple_while_loop.rs:15:5 in function simple_loop_with_loop_invariants
...
### Semantics
A loop invariant expands to several assumptions and assertions:
...
To illustrate the key idea, we show how Kani abstracts the loop in `simple_loop_with_loop_invariants` as a non-loop block:
``` Rust
...
`!guard && inv`, which is `x <= 1 && x >= 1` in the example. The postcondition implies `x == 1`—the property we want to prove at the end of `simple_loop_with_loop_invariants`.
...
There are two extra variables that can be used in loop invariants for `for` loops:
1. `kani_index` : the position (index) of the current iteration in the iterator.
2. `kani_iter_len` : the length of the iterator.

Note that the two extra variables above are only associated with the `for` loop right below the loop invariant.
...
fn forloop() {
    ...
    #[kani::loop_invariant( i == kani_index && i <= kani_iter_len && sum <= (kani_index as u32 * 29) )]
...
## loop invariants inside functions with contracts 
Kani supports using loop invariants together with function-contracts, as demonstrated in the following example:
``` Rust
#![feature(proc_macro_hygiene)]
#![feature(stmt_expr_attributes)]
...
When loop invariants and function-contracts are both enabled (by flags `-Z loop-contracts -Z function-contracts`), 
...
loop invariants comes with the following limitations.
...
4. We don't check if loop invariants are side-effect free. A loop invariant with a side effect could lead to an unsound proof result. Make sure that the specified loop invariants are side-effect free.
