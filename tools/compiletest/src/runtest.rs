...
use crate::common::{CargoKani, Expected, Kani, KaniFixme, Stub, Ui};
...
impl<'test> TestCx<'test> {
    ...
    fn run_revision(&self) {
        match self.config.mode {
            ...
            Expected => self.run_expected_test(true),
            Stub => self.run_stub_test(),
            Ui => self.run_expected_test(false),
        }
    }
    ...
    fn run_cargo_kani_test(&self) {
        ...
        self.verify_output(&proc_res, &expected, true);
    }
    ...
    fn run_expected_test(&self, old_format: bool) {
        let proc_res = self.run_kani();
        let expected =
            fs::read_to_string(self.testpaths.file.parent().unwrap().join("expected")).unwrap();
        self.verify_output(&proc_res, &expected, old_format);
    }
    ...
    /// lines.
    fn verify_output(&self, proc_res: &ProcRes, expected: &str, old_format: bool) {
        // Include the output from stderr here for cases where there are exceptions
        let output = if old_format {
