...
pub fn build_lib() {
    ...
    //  Copy Kani libraries to inside sysroot top folder.
    copy_libs(&artifacts, &sysroot_lib, &is_kani_lib);
    //  Copy Kani libraries to inside rustlib/<target>/lib/ folder.
    copy_libs(&artifacts, &std_path, &is_std_lib);
}

/// Is this artifact a library.
...
/// Copy the `rlib` and the `.so|dylib` files from the artifacts that match the given filter to
/// the target folder.
fn copy_libs<P>(artifacts: &[Artifact], target: &Path, filter: P)
where
    P: FnMut(&Artifact) -> bool,
{
    assert!(target.is_dir(), "Expected a folder, but found {}", target.display());
    for artifact in artifacts.iter().cloned().filter(filter) {
        artifact
            .filenames
            .iter()
            .filter(|path| {
