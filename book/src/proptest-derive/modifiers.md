...
the type of the item being filtered.
...
Usable on: generic type declarations

Normally, when `#[derive(Arbitrary)]` is applied to an item with generic type
arguments, every type argument is required to `impl Arbitrary`. For example,
...
Placing `#[proptest(no_bound)]` on a generic type argument suppresses this. For
...
In order for `no_bound` to be useful, one of two things needs to be true:
...
proptest! {
    #[test]
    fn test_something(wc in any::<WidgetCollection>(WidgetRange(10, 20))) {
...
`weight=3` variant is 50% more likely to be generated than a `weight=2` variant
and three times as likely to be generated as a `weight=1` variant.
