...
fn main() {
    {
        ...
        assert!(data[0] == LCR_DLAB_BIT as u8);
...

    {
        ...
        // assert!(intr_evt.read().unwrap() == 2);
        let mut data = [0u8];
        serial.read(u64::from(IER), &mut data[..]);
        assert!(data[0] & IER_FIFO_BITS == IER_THR_BIT);
        serial.read(u64::from(IIR), &mut data[..]);
        assert!(data[0] & IIR_THR_BIT != 0);
    }
}
