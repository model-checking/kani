...
// length, bounded by the defaults AUTOHARNESS_SLICE_BOUND (16) for slices and
// AUTOHARNESS_STR_BOUND (4) for strings, backed by nondeterministic harness-local storage,
// c.f. the `AnySliceRef` and `AnyStrRef` models. The "TEST NOTE" comments explain the expected
// result per function.
