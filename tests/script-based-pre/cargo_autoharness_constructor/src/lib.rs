...
// A private-field type whose only constructor is an `_unchecked` builder asserting its
// preconditions. Generation cannot use it (such constructors are excluded, since calling them
// with nondeterministic arguments manufactures false alarms), so it must fall back to raw
// field synthesis -- and must NOT be marked "(ctor)", which would claim an under-approximation
// that did not happen.
pub struct OnlyUnchecked {
    v: u16,
}

impl OnlyUnchecked {
    #[doc(hidden)]
    pub fn from_parts_unchecked(v: u16) -> OnlyUnchecked {
        assert!(v >= 1 && v <= 366, "precondition");
        OnlyUnchecked { v }
    }
    pub fn get(&self) -> u16 {
        self.v
