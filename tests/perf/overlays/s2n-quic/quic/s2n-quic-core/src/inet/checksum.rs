...
mod tests {
    ...
    // The kani case uses a deliberately smaller LEN than upstream. The
    // bolero/Kani state space for `differential` scales with LEN; with LEN=16
    // and the post-2025-12-04 nightly, peak RSS exceeds the 16 GB GH-runner
    // ceiling (see `tests/perf/overlays/README.md` for the overlay rationale).
    // LEN=8 keeps the same property under verification but with a state space
    // that fits in CI. The non-Kani (miri) value is preserved.
    #[cfg(any(kani, miri))]
    const LEN: usize = if cfg!(kani) { 8 } else { 32 };
