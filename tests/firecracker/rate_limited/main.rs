...
impl RateLimiter {
    ...
    fn activate_timer(&mut self, timer_state: TimerState) {
        ...
        // DSN self.timer_fd.set_state(timer_state, SetTimeFlags::Default);
        // DSN self.timer_active = true;
    }
...
struct TimerFd;
impl TimerFd {
    pub fn as_raw_fd(&self) -> i32 {panic!()}
    pub fn new_custom(
        clock: ClockId,
        nonblocking: bool,
        cloexec: bool
...
static mut MAX_DELAY : Option<u64> = None;

pub fn kani_sleep(delay: u64) {
    unsafe {
        MIN_DELAY = Some(delay)
    }
}

pub fn kani_set_max_delay(delay: u64) {
    unsafe {
        MAX_DELAY = Some(delay)
    }
}

pub fn kani_reset_max_delay() {
    unsafe {
        MAX_DELAY = None
    } 
}
...
pub(crate) mod kani_proofs {
    ...
    #[kani::proof]
    #[kani::unwind(1)]
    fn test_token_bucket_error() {
        let size = kani::any();
        let one_time_burst = kani::any();
        let complete_refill_time_ms = kani::any();
        kani::assume(size == 0 || complete_refill_time_ms == 0);
        // Verify invalid bucket configurations result in `None`.
        assert!(TokenBucket::new(size, one_time_burst, complete_refill_time_ms).is_none());
    }

    //12 for max of 100
    //17 works for max of 1000
    //21 works for 10000
    //25 works for 100K
    //28 fails for 1Million, and 29 takes a while
    #[kani::proof]
    #[kani::unwind(25)]
    fn test_token_bucket_create_success() {
        let MAX = 100_000;
        let size = kani::any();
        let one_time_burst = kani::any();
        let complete_refill_time_ms = kani::any();
        kani::assume(size < MAX);
        kani::assume(complete_refill_time_ms < MAX);
        kani::assume(size != 0 && complete_refill_time_ms != 0);
        // Verify valid bucket configurations result in `Some`.
        let before = Instant::now();
        let tb = TokenBucket::new(size, one_time_burst, complete_refill_time_ms).unwrap();
    ...
    fn test_token_bucket_reduce() {
        ...
        kani_set_max_delay(400 * NANOSEC_IN_ONE_MILLISEC);
        ...
        assert_eq!(tb.reduce(500), BucketReduction::Success);
        kani_sleep(1000*NANOSEC_IN_ONE_MILLISEC);
