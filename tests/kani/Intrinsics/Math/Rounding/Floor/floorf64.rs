...
fn test_one() {
    let one = 1.0;
    let ceil_res = unsafe { floorf64(one) };
    assert!(ceil_res == 1.0);
}
...
fn test_one_frac() {
    let one_frac = 1.2;
    let ceil_res = unsafe { floorf64(one_frac) };
    assert!(ceil_res == 1.0);
}
...
fn test_one_neg() {
    let one_neg = -1.8;
    let ceil_res = unsafe { floorf64(one_neg) };
    assert!(ceil_res == -2.0);
}
...
fn test_conc() {
    let conc = -42.6;
    let ceil_res = unsafe { floorf64(conc) };
    assert!(ceil_res == -43.0);
}
...
fn test_towards_inf() {
    ...
    let ceil_res = unsafe { floorf64(x) };
    assert!(ceil_res <= x);
}
