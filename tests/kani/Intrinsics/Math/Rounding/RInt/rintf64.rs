...
// Checks that `rintf64` returns the nearest integer to the argument. The
// default rounding mode is rounding half to even, which is described here:
// https://en.wikipedia.org/wiki/Rounding#Round_half_to_even
//
// `rintf64` works like `nearbyintf64`, but it may raise an inexact
// floating-point exception which isn't supported in Rust:
// https://github.com/rust-lang/rust/issues/10186
// So in practice, `rintf64` and `nearbyintf64` work the same way.
#![feature(core_intrinsics)]
use std::intrinsics::rintf64;

#[kani::proof]
fn test_one() {
    let one = 1.0;
    let result = unsafe { rintf64(one) };
    assert!(result == 1.0);
}
...
fn test_one_frac() {
    let one_frac = 1.9;
    let result = unsafe { rintf64(one_frac) };
    assert!(result == 2.0);
}
...
fn test_conc() {
    let conc = -42.6;
    let result = unsafe { rintf64(conc) };
    assert!(result == -43.0);
}
