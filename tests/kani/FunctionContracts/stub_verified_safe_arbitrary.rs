...
//! `tests/expected/function-contract/stub_verified_arbitrary_cycle.rs`, which
//! covers the case where the `Arbitrary` impl does reach the stubbed function
//! and Kani reports the resulting cycle. Here the derived `Arbitrary` builds
//! `MyType` field by field, so no cycle exists and verification succeeds.
