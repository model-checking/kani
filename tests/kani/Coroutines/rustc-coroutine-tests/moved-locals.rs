...
fn main() {
    ...
    assert_eq!(
        unsafe { Pin::new_unchecked(&mut coroutine) }.resume(()),
    ...
    let mut coroutine = move_before_yield_with_noop();
    assert_eq!(
        unsafe { Pin::new_unchecked(&mut coroutine) }.resume(()),
        CoroutineState::Yielded(())
    );
    ...
    let mut coroutine = overlap_move_points();
    assert_eq!(
        unsafe { Pin::new_unchecked(&mut coroutine) }.resume(()),
        CoroutineState::Yielded(())
    );
    ...
    let mut coroutine = overlap_x_and_y();
    assert_eq!(
        unsafe { Pin::new_unchecked(&mut coroutine) }.resume(()),
        CoroutineState::Yielded(())
    );
