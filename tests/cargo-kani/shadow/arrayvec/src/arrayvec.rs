...
pub(crate) static mut SM: kani::shadow::ShadowMem<bool> = kani::shadow::ShadowMem::new(false);
...
impl<T, const CAP: usize> ArrayVec<T, CAP> {
    ...
    pub fn try_insert(&mut self, index: usize, element: T) -> Result<(), CapacityError<T>> {
        ...
        unsafe { // infallible
            // The spot to put the new value
            {
                ...
                // SHADOW: Check that the elements that are going to be shifted are
                // initialized
                for i in 0..len - index {
                    assert!(SM.get(p.add(i)));
                }
                ...
                // element.
                ptr::write(p, element);
                // SHADOW: mark the newly written element as initialized
                SM.set(p, true);
            }
            self.set_len(len + 1);
        }
    ...
    pub fn pop_at(&mut self, index: usize) -> Option<T> {
        if index >= self.len() {
            ...
            let d = self.drain(index..index + 1).next();
            // SHADOW: Mark the last element as uninitialized
            unsafe {
                SM.set(self.get_unchecked_ptr(self.len()), false);
            }
            d
        }
    }
        ...
        where F: FnMut(&mut T) -> bool
    {
        ...
        impl<T, const CAP: usize> Drop for BackshiftOnDrop<'_, T, CAP> {
            fn drop(&mut self) {
                if self.deleted_cnt > 0 {
                    unsafe {
                        // SHADOW: Check that all the elements that will be copied are initialized
                        for i in 0..self.original_len - self.processed_len {
                            assert!(SM.get(self.v.as_ptr().add(i)));
                        }
                        ptr::copy(
        ...
        fn process_one<F: FnMut(&mut T) -> bool, T, const CAP: usize, const DELETED: bool>(
            ...
            // SHADOW: Check that the element is initialized
            unsafe { assert!(SM.get(cur)) };
            if !f(unsafe { &mut *cur }) {
                g.processed_len += 1;
                g.deleted_cnt += 1;
        ...
        drop(g);
        // SHADOW: Mark the removed tail elements as uninitialized
        for i in self.len as usize..original_len {
            unsafe { SM.set(self.as_ptr().add(i), false) };
        }
    }
        ...
        where T: Copy,
    {
        ...
        unsafe {
            ...
            // SHADOW: Mark all new elements as initialized
            for i in self_len..self_len + other_len {
                SM.set(self.as_ptr().add(i), true);
            }
        }
        Ok(())
    }
    ...
    pub unsafe fn into_inner_unchecked(self) -> [T; CAP] {
        ...
        // SHADOW: Check that all the elements are initialized
        for i in 0..CAP {
            assert!(SM.get(self_.as_ptr().add(i)));
        }
...
impl<T, const CAP: usize> From<[T; CAP]> for ArrayVec<T, CAP> {
    ...
    fn from(array: [T; CAP]) -> Self {
        ...
        unsafe {
            ...
            vec.set_len(CAP);
            // SHADOW: mark elements as initialized
            for i in 0..CAP {
                SM.set(vec.as_ptr().add(i), true);
            }
        }
        vec
    }
...
impl<'a, T: 'a, const CAP: usize> Iterator for Drain<'a, T, CAP> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.iter.next().map(|elt|
            unsafe {
                // SHADOW: Check that element is initialized
                assert!(SM.get(elt as *const T));
...
mod kani_checks {
    ...
    #[kani::proof]
    fn check_push_init() {
        let mut av = ArrayVec::<i32, N>::new();
        for _i in 0..N {
            av.push(kani::any());
        }
        for _i in 0..N {
            assert!(av.pop().is_some());
        }
    }
}
