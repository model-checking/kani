    ...
    F: FnMut(TyCtxt, DefId) -> bool,
{
    ...
    for hir_id in tcx.hir_crate_items(()).items() {
        ...
        if predicate(tcx, def_id) {
            ...
            let mut extrator =
                ConstMonoItemExtractor { tcx, body, collected: FxHashSet::default() };
...
impl<'a, 'tcx> MirVisitor<'tcx> for ConstMonoItemExtractor<'a, 'tcx> {
    fn visit_rvalue(&mut self, rvalue: &Rvalue<'tcx>, location: Location) {
        ...
        match *rvalue {
            Rvalue::Cast(CastKind::Pointer(PointerCast::ClosureFnPointer(_)), ref operand, _) => {
                let source_ty = operand.ty(self.body, self.tcx);
                // Hazard: We don't have an instance, so there's no monomorphize possible here.
                // This should be ok, since we should only be called on a top-level const declaration
                // generated by tests, which should already be monomorphic.
                // But I wanted to note in case someone tries to re-use this code somewhere else that
                // I'm embedding this assumption here and omitting this step:
                //   `let source_ty = self.monomorphize(source_ty);`
                match *source_ty.kind() {
                    Closure(def_id, substs) => {
                        let instance = Instance::resolve_closure(
