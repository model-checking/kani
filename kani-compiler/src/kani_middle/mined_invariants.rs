...
/// Whether `bb` post-dominates `from` in `body` w.r.t. normal returns:
...
fn postdominates(body: &Body, from: usize, bb: usize) -> bool {
    ...
    while let Some(cur) = queue.pop() {
        ...
        match &term.kind {
            ...
            TerminatorKind::Call { target: Some(t), .. } => push(*t),
            TerminatorKind::Drop { target, .. } => push(*target),
            TerminatorKind::Assert { target, .. } => push(*target),
            // Sinks: no normal-return successor, so they cannot reach a `Return` bypassing `bb`.
            TerminatorKind::Unreachable
            | TerminatorKind::Resume
            | TerminatorKind::Abort
            | TerminatorKind::Call { target: None, .. } => {}
            // Any other terminator may carry a normal-return successor we do not model here (e.g.
            // `InlineAsm { destination: Some(_) }`). Conservatively report that `bb` does not
            // post-dominate, so the conjunct is not admitted as an invariant (sound for the
            // assume direction, at worst rejecting a mineable condition).
            _ => return false,
        }
    }
    true
...
/// Whether `p` is a temp holding `&self` (defined once as `Ref(.., self-place)`).
