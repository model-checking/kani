...
impl<'tcx, 'r> GotocCtx<'tcx, 'r> {
    ...
    fn codegen_vector(&mut self, ty: Ty<'tcx>) -> Type {
        ...
        // so a value is built and read through a byte-level reinterpretation of the whole vector
        // (`codegen_rvalue_aggregate` and `codegen_simd_field`) and the lanes are never accessed as
        // pointers at this level.
