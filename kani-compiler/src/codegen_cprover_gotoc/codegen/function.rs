...
use crate::kani_middle::attributes::get_modifies_annotations;
...
impl<'tcx> GotocCtx<'tcx> {
    ...
    pub fn codegen_function(&mut self, instance: Instance<'tcx>) {
        ...
        if old_sym.is_function_definition() {
            ...
            self.generate_contract(&instance);
        }
    ...
    pub fn generate_contract(&mut self, instance: &Instance) {
        if let Some(modifies_clauses) = get_modifies_annotations(self.tcx, instance) {
            self.codegen_modifies_clause(modifies_clauses);
        }
    }
}
