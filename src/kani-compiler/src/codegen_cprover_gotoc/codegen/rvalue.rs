...
impl<'tcx> GotocCtx<'tcx> {
    ...
    pub fn codegen_rvalue_ref(&mut self, place: &Place<'tcx>, result_mir_type: Ty<'tcx>) -> Expr {
        ...
        let projection = unwrap_or_codegen_unimplemented!(self, self.codegen_place(place));
    ...
    pub fn codegen_rvalue_len(&mut self, p: &Place<'tcx>) -> Expr {
        ...
        match pt.kind() {
            ...
            ty::Slice(_) => unwrap_or_codegen_unimplemented!(self, self.codegen_place(p))
                .fat_ptr_goto_expr
                .unwrap()
    ...
    pub fn codegen_rvalue(&mut self, rv: &Rvalue<'tcx>) -> Expr {
        ...
        match rv {
            ...
            Rvalue::Discriminant(p) => {
                let place = unwrap_or_codegen_unimplemented!(self, self.codegen_place(p)).goto_expr;
                let pt = self.place_ty(p);
